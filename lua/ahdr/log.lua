...
local function threshold()
  local name = require("ahdr.config").options.log_level
...
function M.at(level, msg)
  if not require("ahdr.config").options.notify then
    return
  end
  if level < threshold() then
    return
  end
  vim.notify(msg, level, { title = "ahdr" })
end
...
---User-initiated feedback (from an `:Ahdr*` command). Always shown.
---@param msg string
---@param level? integer
function M.echo(msg, level)
  vim.notify(msg, level or vim.log.levels.INFO, { title = "ahdr" })
end
